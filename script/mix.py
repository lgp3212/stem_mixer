...
def generate(data_home, sr, duration, invalid_mixture, stretched_audios):
    ...
    if invalid_mixture == False:  # if it passes all checks
        ...
        for k in range(0, len(truncated_stems)):
            sf.write(
                f"{individual_output_folder}/stem{k+1}.wav", truncated_stems[k], sr
            )
...
def select_base_track(data_home, n_stems, n_harmonic, n_percussive):
    ...
    if (
        n_harmonic == 0 and n_percussive == 0
    ):  # if default provided, ensuring n_harm and n_perc values are set
        n_harmonic = n_stems // 2
        n_percussive = n_stems - n_harmonic

    if (
        n_percussive > 0
    ):  # checking to see if they have any percussive stems so we can choose our base
        percussive = True
    else:
        percussive = False
    ...
    for file in json_files:  # splitting jsons into harmonic and percussive
        with open(file, "r") as f:
            ...
            if data.get("sound_class") == "percussive":
                ...
                data.get("sound_class") == "harmonic"
                or data.get("sound_class") == "vocals"
            ):
                json_harmonic.append(file)

    audio_files = glob.glob(os.path.join(path_to_stems, "*.wav")) + glob.glob(
        os.path.join(path_to_stems, "*.mp3")
    )
...
def select_top_tracks(
        ...
        with open(file, "r") as f:
            ...
            if data.get("tempo_bin") == tempo_bin:
                ...
                if new_stem_name != base_stem_name:
                    ...
                    tempo_bin_percussive[new_stem_name] = [
                        data.get("tempo"),
    ...
    for file in json_harmonic:
        with open(file, "r") as f:
            ...
            if data.get("tempo_bin") == tempo_bin:
                ...
                new_stem_name, _ = os.path.splitext(split_name)
                if new_stem_name != base_stem_name:
                    tempo_bin_harmonic[new_stem_name] = [
                        data.get("tempo"),
                        data.get("instrument_name"),
                    ]
    ...
    try:
        ...
        if len(list_of_instruments) != len(
            set(list_of_instruments)
        ):  # repeated instrument
            invalid_mixture = True
    ...
    return (
        ...
        base_tempo,
        invalid_mixture,
    )  # should we make it so this process cuts quicker
    # when invalid
